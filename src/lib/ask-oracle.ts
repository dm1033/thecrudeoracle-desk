...
export const askOracle = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    if (!apiKey) return { ok: false as const, error: "Oracle is dark in this environment." };
    ...
    if (!res.ok) return { ok: false as const, error: `Desk error ${res.status}` };
